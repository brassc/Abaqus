...
Default reference calibration (override by setting variables before execfile):
    REFERENCE_PRELOAD        = 0.3      preload that worked at the reference site
...
CORD_SET_NAME notes:
    - Set to the name of an assembly node set containing all cord nodes (e.g. 'Cord').
...
print("sets_scaled_inpmod_gm_script.py loaded")
...
# --- Cord node set filter ---
# Set CORD_SET_NAME to the name of an assembly node set containing all cord nodes.
# If not set, all nodes of INSTANCE_NAME are classified (original behaviour).
...
if 'MODEL_NAME' not in dir():
    ...
    if CORD_SET_NAME:
        print("Cord node set filter: " + CORD_SET_NAME)
    else:
        print("Cord node set filter: (none - all instance nodes)")
